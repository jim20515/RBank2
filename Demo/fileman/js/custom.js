...
function FileSelected(file){
   ...
   */
    ...
    $parent.find('#' + fieldId).val(file.fullPath);

    if ($parent.find('#' + fieldId + '_Preview').length) {

        $parent.find('#' + fieldId + '_Preview').attr('src', file.fullPath + '?w=300');
        $parent.find('#box-' + fieldId + ' .box-title').text(file.name);
        window.parent.closeModal_PreviewImage();

    } else {

        //window.parent.closeModal();
        if (window.parent.SortableModal_API && typeof window.parent.SortableModal_API.closeModal === 'function') {
            window.parent.SortableModal_API.closeModal();
        }
    }
